...
frequencyDistribution <- function(){
    ...
    onOK <- function(){
        ...
        if (goodnessOfFit == 1){
            ...
            levs <- eval(parse(text=paste("levels(factor(", .activeDataSet, "$", x, "))", sep="")))
            n.levs <- length(levs)
...
statisticsTable <- function(){
    ...
    groupBox <- variableListBox(variablesFrame, Variables(), selectmode="multiple", title=gettextRcmdr("Factors (pick one or more)"))
